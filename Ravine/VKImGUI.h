...
public:
	...
	ImGUI(VkDevice *logicalDevice) : logicalDevice(logicalDevice)
	{
		ImGui::CreateContext();
	};
	...
	void initResources(VkRenderPass renderPass, VkQueue copyQueue)
	{
		...
		VK_CHECK_RESULT(vkCreateImage(*logicalDevice, &imageInfo, nullptr, &fontImage));
		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(*logicalDevice, fontImage, &memReqs);
		VkMemoryAllocateInfo memAllocInfo = vks::initializers::memoryAllocateInfo();
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(*logicalDevice, &memAllocInfo, nullptr, &fontMemory));
		VK_CHECK_RESULT(vkBindImageMemory(*logicalDevice, fontImage, fontMemory, 0));
		...
		VK_CHECK_RESULT(vkCreateImageView(*logicalDevice, &viewInfo, nullptr, &fontView));
