...
__inline std::array<std::uint32_t, 256> generate_crc_lookup_table() noexcept
{
...
__inline uint32_t checkSum(uint32_t checksum, char value, const std::array<std::uint32_t, 256>& table)
{
...
__inline std::uint32_t crc(char* first, size_t size, uint32_t initialVal = ~uint32_t{ 0 } &uint32_t{ 0xFFFFFFFFuL })
{
	// Generate lookup table only on first use then cache it - this is thread-safe.
	static const std::array<uint32_t, 256> table = generate_crc_lookup_table();
