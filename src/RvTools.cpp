...
namespace rvTools
{
	...
	RvTexture createTexture(RvDevice* device, void* pixels, uint32_t width, uint32_t height, VkFormat format)
	{
		...
		RvDynamicBuffer stagingBuffer =
		    device->createDynamicBuffer(dataSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		...
		*/
		...
		device->createImage(extent, mipLevels, VK_SAMPLE_COUNT_1_BIT, format, VK_IMAGE_TILING_OPTIMAL,
		...
		transitionImageLayout(*device, texture.handle, format, VK_IMAGE_LAYOUT_UNDEFINED,
				      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);

		// Transfering buffer data to image object
		copyBufferToImage(device, stagingBuffer.handle, texture.handle, static_cast<uint32_t>(width),
		...
		texture.view = createImageView(device->handle, texture.handle, format, VK_IMAGE_ASPECT_COLOR_BIT,
					       texture.mipLevels);

		return texture;
	}

	static void generateMipmap(RvDevice* device, VkImage image, VkFormat imageFormat, uint32_t texWidth,
	...
	static void copyBufferToImage(RvDevice* device, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
	{
		VkCommandBuffer commandBuffer = device->beginSingleTimeCommands();
	...
	static EShLanguage FindLanguage(const int shaderStageBit)
	{
		for (int i = 0; i < EShLanguage::EShLangCount; i++)
		{
			if ((1 << i) == shaderStageBit)
				return (EShLanguage)i;
		}
	...
	{
		EShLanguage stage = FindLanguage(shaderStageBit);
		...
		const char* shaderStrs[1]{pshader.data()};
		const int shaderStrSizes = static_cast<int>(pshader.size());
		shader.setStringsWithLengths(shaderStrs, &shaderStrSizes, 1);

		if (!shader.parse(&Resources, 100, false, messages))
		{
			fmt::print(stdout, shader.getInfoLog());
			fmt::print(stdout, shader.getInfoDebugLog());
		...
		vector<char> spirv;
		std::vector<unsigned int> stlBuff;
		glslang::GlslangToSpv(*program.getIntermediate(stage), stlBuff);
		size_t buffSize = stlBuff.size() * sizeof(unsigned int);
		spirv.resize(buffSize);
		memcpy(spirv.data(), stlBuff.data(), buffSize);
		return spirv;
	}
				   ...
				   VkImageLayout newLayout, uint32_t mipLevels)
	{
			 ...
			 newLayout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
		{
			...
			destinationStage =
			    VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT; // Phase where buffer reading occurs
		}
	...
	void copyToMemory(RvDevice& device, char* data, VkDeviceSize size, const RvDynamicBuffer& dynamicBuffer)
	{
		...
		vkMapMemory(device.handle, dynamicBuffer.memory, 0, dynamicBuffer.allocatedSize, 0, reinterpret_cast<void**>(&gpuBuffer));
		memcpy(gpuBuffer, data, size);
		VkMappedMemoryRange range = {};
		range.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
		range.memory = dynamicBuffer.memory;
		range.size = VK_WHOLE_SIZE;
		vkFlushMappedMemoryRanges(device.handle, 1, &range);
		vkUnmapMemory(device.handle, dynamicBuffer.memory);
	}
							    ...
							    const RvFramebufferAttachmentCreateInfo& createInfo)
	{
		...
		device.createImage(createInfo.extent, createInfo.mipLevels, device.sampleCount, createInfo.format,
				   createInfo.tilling, createInfo.usage, createInfo.memoryProperties,
				   createInfo.createFlag, newAttachment.image, newAttachment.memory);
		newAttachment.imageView = createImageView(device.handle, newAttachment.image, createInfo.format,
							  createInfo.aspectFlag, createInfo.mipLevels);
		transitionImageLayout(device, newAttachment.image, createInfo.format, createInfo.initialLayout,
				      createInfo.finalLayout, createInfo.mipLevels);
